...
function countLetters(argument){
   ...
   var letterObj = {};
     for (var i = 0; i < joinArg.length; i++){
        var letter = joinArg[i] ;  
         letterObj[letter] = [];
     }
      for (var i = 0; i < joinArg.length; i++){
         var letter = joinArg[i];
         letterObj[letter].push(i) 
   //      if (letterObj[letter]) {
   //      letterObj[letter] += letter.indexOf(letter[i]);
   //      console.log("return after if:", letterObj)
   //      } else { letterObj[letter]; 
   //       //console.log("return after else:", letterObj)// second index here
   //      }
    }
     return letterObj;
 }
